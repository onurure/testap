...
const apiVersion = 'https://kitakitap.soneyazilim.com/ota/update.json';
export const useCheckVersion = () => {
  ...
  const startUpdate = async (url: string, version: number) => {
    hotUpdate.downloadBundleUri(ReactNativeBlobUtil, url, version, {
      updateSuccess: () => {
        Alert.alert('Update success!', 'Restart to apply the changing', [
          {
            text: 'ok',
  ...
  const onCheckVersion = () => {
    fetch(apiVersion).then(async data => {
      ...
      console.log('🚀 ~ fetch ~ currentVersion:', currentVersion);
  ...
  const onCheckGitVersion = () => {
    ...
    hotUpdate.git.checkForGitUpdate({
      ...
      url: 'https://github.com/onurure/testap.git',
